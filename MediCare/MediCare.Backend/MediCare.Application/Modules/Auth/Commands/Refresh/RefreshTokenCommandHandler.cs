namespace MediCare.Application.Modules.Auth.Commands.Refresh;
    ...
    : IRequestHandler<RefreshTokenCommand, RefreshTokenCommandDto>
{
    public async Task<RefreshTokenCommandDto> Handle(RefreshTokenCommand request, CancellationToken ct)
    {
        ...
        if (rt is null || rt.ExpiresAtUtc <= nowUtc)
            throw new MediCareConflictException("Refresh token je nevažeći ili je istekao.");
        ...
        {
            throw new MediCareConflictException("Neispravan klijentski otisak.");
        }

        var user = rt.User;
        if (user is null || !user.IsEnabled || user.IsDeleted)
            throw new MediCareConflictException("Korisnički nalog je nevažeći.");
